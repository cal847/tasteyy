...
def is_serializer(obj, strict=False) -> TypeGuard[_SerializerType]:
    ...
    return isinstance(force_instance(obj), serializers.BaseSerializer) or (
        bool(OpenApiSerializerExtension.get_match(obj)) and not strict
    )
...
def get_list_serializer(obj: Any):
    return (
        force_instance(obj)
        if is_list_serializer(obj)
        else get_class(obj)(many=True, context=obj.context)
    )
...
def is_patched_serializer(serializer, direction) -> bool:
    return bool(
        ...
        and not (
            spectacular_settings.COMPONENT_SPLIT_REQUEST and direction == "response"
        )
    )
...
def build_basic_type(
    obj: Union[_KnownPythonTypes, OpenApiTypes]
...
def build_array_type(
    schema: _SchemaType, min_length=None, max_length=None
) -> _SchemaType:
...
def build_choice_field(field) -> _SchemaType:
    choices = list(
        OrderedDict.fromkeys(field.choices)
    ...
    schema["x-spec-enum-id"] = list_hash(
        [(k, v) for k, v in field.choices.items() if k not in ("", None)]
    )
...
def _follow_field_source(model, path: List[str]):
    ...
    if len(path) == 1:
        ...
        if isinstance(
            field_or_property, (property,) + CACHED_PROPERTY_FUNCS
...
class ComponentRegistry:
    ...
    def __contains__(self, component):
        ...
        suppress_collision_warning = get_override(
            registry_id, "suppress_collision_warning", False
...
class OpenApiGeneratorExtension(Generic[T], metaclass=ABCMeta):
    ...
    def _load_class(cls):
        try:
            ...
            if (
                any(cls.target_class.startswith(app + ".") for app in installed_apps)
                and not cls.optional
            ):
                warn(
...
def list_hash(lst: Any) -> str:
    return hashlib.sha256(
        json.dumps(list(lst), sort_keys=True, cls=JSONEncoder).encode()
    ).hexdigest()[:16]
...
def resolve_django_path_parameter(path_regex, variable, available_formats):
    ...
    for match in _PATH_PARAMETER_COMPONENT_RE.finditer(path_regex):
        ...
        if api_settings.SCHEMA_COERCE_PATH_PK and parameter == "pk":
            parameter = "id"
        elif spectacular_settings.SCHEMA_COERCE_PATH_PK_SUFFIX and parameter.endswith(
            "_pk"
        ):
        ...
        if converter.startswith("drf_format_suffix_"):
            ...
            enum_values = [
                f".{suffix}"
                for suffix in explicit_formats
        ...
        if converter in spectacular_settings.PATH_CONVERTER_OVERRIDES:
            ...
            error(
                f'Encountered path converter "{converter}" that is unknown to Django.'
            )
            return None
...
def modify_for_versioning(patterns, method, path, view, requested_version):
    ...
    if issubclass(view.versioning_class, versioning.URLPathVersioning):
        ...
        if not hasattr(view.request, "accepted_renderer"):
            ...
            view.request.accepted_renderer, view.request.accepted_media_type = (
                negotiated
            )
        media_type = _MediaType(view.request.accepted_media_type)
        view.request.accepted_media_type = f"{media_type.full_type}; {view.versioning_class.version_param}={requested_version}"

    return path


def modify_media_types_for_versioning(view, media_types: List[str]) -> List[str]:
    if not view.versioning_class or not issubclass(
        view.versioning_class, versioning.AcceptHeaderVersioning
    ):
        return media_types
...
def detype_pattern(pattern):
    ...
    if isinstance(pattern, URLResolver):
        ...
        return RegexPattern(
            regex=detyped_regex, name=pattern.name, is_endpoint=pattern._is_endpoint
        )
    else:
        warn(
            f'unexpected pattern "{pattern}" encountered while simplifying urlpatterns.'
        )
        return pattern
...
def sanitize_result_object(result):
    ...
    for operation_id, paths in operations.items():
        if len(paths) == 1:
            continue
        warn(
            f'operationId "{operation_id}" has collisions {paths}. resolving with numeral suffixes.'
        )
...
def camelize_operation(path, operation):
    for path_variable in re.findall(r"\{(\w+)\}", path):
        path = path.replace(
            f"{{{path_variable}}}", f"{{{inflection.camelize(path_variable, False)}}}"
        )
...
def resolve_type_hint(hint):
    ...
    if origin is None and is_basic_type(hint, allow_none=False):
        ...
        origin is dict
        or origin is defaultdict
        or origin is OrderedDict
        or hint is ReturnDict
    ):
...
def whitelisted(
    obj: object, classes: Optional[List[Type[object]]], exact=False
) -> bool:
    if classes is None:
        return True
    if exact:
...
def filter_supported_arguments(func, **kwargs):
    ...
    return {arg: val for arg, val in kwargs.items() if arg in sig.parameters}
